...
.auth-title { font-size: 1.6rem; font-weight: 800; color: #fff; text-align: center; margin-bottom: 6px; }
.auth-subtitle { color: rgba(255,255,255,0.55); font-size: 0.88rem; text-align: center; margin-bottom: 28px; }

.auth-label { font-size: 0.75rem; font-weight: 700; color: rgba(255,255,255,0.6); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 7px; }
.auth-input {
    width: 100%; padding: 13px 16px 13px 44px;
    background: rgba(255,255,255,0.08);
    border: 1.5px solid rgba(255,255,255,0.12);
    border-radius: 12px; color: #fff; font-size: 0.92rem;
    outline: none; transition: all 0.2s; margin-bottom: 18px;
}
...
.auth-input-icon { position: absolute; left: 14px; top: 14px; color: rgba(255,255,255,0.4); font-size: 0.9rem; }

.auth-btn {
    width: 100%; padding: 14px; border: none; border-radius: 12px;
    background: linear-gradient(135deg,#6366f1,#8b5cf6);
    color: #fff; font-weight: 700; font-size: 1rem; cursor: pointer;
    box-shadow: 0 6px 20px rgba(99,102,241,0.4); transition: all 0.3s;
    margin-top: 4px;
}
...
.auth-divider span { color: rgba(255,255,255,0.4); font-size: 0.78rem; }
...
.auth-footer { text-align: center; margin-top: 24px; font-size: 0.88rem; color: rgba(255,255,255,0.5); }
.auth-footer a { color: #818cf8; font-weight: 700; text-decoration: none; }
.auth-footer a:hover { color: #a5b4fc; }

.error-msg { background: rgba(239,68,68,0.15); border: 1px solid rgba(239,68,68,0.3); border-radius: 10px; padding: 10px 14px; color: #fca5a5; font-size: 0.82rem; margin-bottom: 16px; }
</style>
@endpush
...
<div class="auth-page">
    <div class="auth-box">
        ...
        <div class="auth-title">እንኳን ደህና መጡ 👋</div>
        <div class="auth-subtitle">ወደ መለያዎ ለመግባት ኢሜይልዎን ያስገቡ</div>

        @if(session('error'))
            <div class="error-msg"><i class="fas fa-exclamation-circle me-2"></i>{{ session('error') }}</div>
        @endif
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            @error('email')<div style="color:#fca5a5;font-size:0.8rem;margin-top:-14px;margin-bottom:14px;">{{ $message }}</div>@enderror
            ...
            @error('password')<div style="color:#fca5a5;font-size:0.8rem;margin-top:-14px;margin-bottom:14px;">{{ $message }}</div>@enderror

            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                <label style="display:flex;align-items:center;gap:8px;font-size:0.82rem;color:rgba(255,255,255,0.5);cursor:pointer;">
                    <input type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }} style="accent-color:#6366f1;">
                    አስታውሰኝ
                </label>
                @if(Route::has('password.request'))
                    <a href="{{ route('password.request') }}" style="font-size:0.82rem;color:#818cf8;text-decoration:none;">የይለፍ ቃል ረሱ?</a>
                @endif
            </div>

            <button type="submit" class="auth-btn"><i class="fas fa-sign-in-alt me-2"></i>ይግቡ</button>
        </form>
