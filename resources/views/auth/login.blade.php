    ...
    <style>
        * { font-family: 'Inter', sans-serif; box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body {
            ...
            display: flex; align-items: flex-start; justify-content: center; padding: 16px 20px;
        ...
        .soc-btn {
            ...
            font-size:0.75rem;font-weight:600;color:#fff;text-decoration:none;transition:all 0.2s;
        }
...
<body>
    <div class="card">
        <div class="brand">
            ...
            <div class="brand-name">ተሸሾፕ</div>
        </div>

        <h2>እንኳን ደህና መጡ 👋</h2>
        <p class="sub">ወደ መለያዎ ለመግባት ኢሜይልዎን ያስገቡ</p>

        @if(session('error'))
            <div class="err"><i class="fas fa-exclamation-circle me-2"></i>{{ session('error') }}</div>
        @endif
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            @error('email')<div style="color:#fca5a5;font-size:0.78rem;margin-top:-14px;margin-bottom:12px;">{{ $message }}</div>@enderror
            ...
            @error('password')<div style="color:#fca5a5;font-size:0.78rem;margin-top:-14px;margin-bottom:12px;">{{ $message }}</div>@enderror

            <div class="remember-row">
                <label><input type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }} style="accent-color:#6366f1;"> አስታውሰኝ</label>
                @if(Route::has('password.request'))
                    <a href="{{ route('password.request') }}">የይለፍ ቃል ረሱ?</a>
                @endif
            </div>

            <button type="submit" class="btn-main"><i class="fas fa-sign-in-alt me-2"></i>ይግቡ</button>
        </form>
